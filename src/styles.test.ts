...
test('fits production impacts inside faction boundaries', () => {
  ...
  expect(css).toContain('grid-template-columns: repeat(3, minmax(54rem, 1fr));');
  expect(css).toContain('column-gap: .75rem;');
  expect(css).toContain('grid-template-columns: minmax(0, 1fr) 4.3rem 4rem minmax(0, 1fr) 8.5rem;');
