...
type Props = { faction: Faction; state: PlanFactionState; resolved: ResolvedPopulationTarget | null; onChange: (state: PlanFactionState) => void };
const entry = (raw: string) => ({ raw, value: parseNonNegativeInteger(raw) });

export function GrowthTargetFaction({ faction, state, resolved, onChange }: Props) {
  ...
  return <details className={`growth-target growth-target--${faction}`} open>
    <summary>{config.label} · {state.intent.kind === 'follow'
      ? state.intent.tierMode === 'mirror'
        ? 'Follow islands · Mirror actual tiers'
        : 'Follow islands · Unrestricted ascension'
      : state.intent.kind === 'residences' ? 'By residences' : 'By population'}</summary>
    ...
    {state.intent.kind === 'follow' && <div className="growth-target__follow-modes">
      <button type="button" aria-pressed={state.intent.tierMode === 'mirror'} aria-label={`Mirror ${config.label} actual tiers`} onClick={() => onChange({ ...state, intent: { kind: 'follow', tierMode: 'mirror' } })}>Mirror actual tiers</button>
      <button type="button" aria-pressed={state.intent.tierMode === 'unrestricted'} aria-label={`Project ${config.label} through unrestricted ascension`} onClick={() => onChange({ ...state, intent: { kind: 'follow', tierMode: 'unrestricted' } })}>Unrestricted ascension</button>
    </div>}
